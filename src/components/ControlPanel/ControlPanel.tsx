...
export function ControlPanel() {
  ...
  const setSudokuGridValue = (value: string | number): void => {
    ...
    dispatch(
      setCellValue({ row: activePosition.row, col: activePosition.col, value })
    );
  ...
  const clearCell = (): void => {
    if (activePosition.isReadOnly === "false") {
      if (isEditmode) {
        dispatch(
          clearPencilCel({ row: activePosition.row, col: activePosition.col })
        );
      } else {
        dispatch(
          clearCellValue({ row: activePosition.row, col: activePosition.col })
        );
  ...
  return (
    <StyledGameControlWrapper>
      <StyledNumpadWrapper>
        <StyledNumpad>{getnumpadItems()}</StyledNumpad>
      </StyledNumpadWrapper>
      <StyledGameControlsButtons>
        <StyledButton onClick={clearCell}>Cancel</StyledButton>
        <StyledButton isEditode={isEditmode} onClick={setEditMode}>
          Notes
        </StyledButton>
