...
import { SudokuGrid } from "../slices/sudokuGridSlice";
...
export class SudokuService {
  ...
  private makeSmallRowsSwap(grid: number[][]): number[][] {
    ...
    const numberOfFirstRow = this.getRandomNumber(
      rangeOfRows[0],
      rangeOfRows[1]
    );
    let numberOfSecondRow = this.getRandomNumber(
      rangeOfRows[0],
      rangeOfRows[1]
    );
  ...
  private makeBigRowsSwap(grid: number[][]): number[][] {
    ...
    const arraysAndInexes = createMap(
      secondRowRangeOfArea.concat(firstRowRangeOfArea),
      firstArrayToSwap.concat(secondArrayToSwap)
    );
    for (let i = 0; i < Math.pow(this.size, 2); i++) {
  ...
  private mixGrid = (grid: number[][]): number[][] => {
    ...
    let mixingFunctions = [
      this.transposing,
      this.makeSmallRowsSwap,
      this.makeSmallCollumSwap,
      this.makeBigRowsSwap,
  ...
  private hideCells(grid: (string | number)[][]) {
    ...
    while (cellsHaveToBeEmpty.size <= this.maxEmptyCells) {
      ...
      cellsHaveToBeEmpty.set(`x:${firstIndex}-y:${secondIndex}`, [
        firstIndex,
        secondIndex,
      ]);
    }
  ...
  private getRangeArray(areaNumber: number, isForBig?: boolean): number[] {
    ...
    for (
      let i = areaNumber * this.size;
      i <= (areaNumber + 1) * this.size - 1;
      i++
    ) {
      range.push(i);
    }
